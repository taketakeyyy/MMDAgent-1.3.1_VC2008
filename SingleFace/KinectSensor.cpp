...
HRESULT KinectSensor::GetVideoConfiguration(FT_CAMERA_CONFIG* videoConfig)//ビデオ設定を取得する
{
    if (!videoConfig)
    {
        return E_POINTER;
...
HRESULT KinectSensor::GetDepthConfiguration(FT_CAMERA_CONFIG* depthConfig)//深度設定を取得する
{
    if (!depthConfig)
    {
        return E_POINTER;
...
HRESULT KinectSensor::Init(NUI_IMAGE_TYPE depthType, NUI_IMAGE_RESOLUTION depthRes, BOOL bNearMode, BOOL bFallbackToDefault, NUI_IMAGE_TYPE colorType, NUI_IMAGE_RESOLUTION colorRes, BOOL bSeatedSkeletonMode)
{
    ...
    Release(); // Deal with double initializations.（二重初期化）
...
void KinectSensor::GotSkeletonAlert()
{
    ...
    for( int i = 0 ; i < NUI_SKELETON_COUNT ; i++ )
    {
        ...
        {
            m_SkeletonTracked[i] = true;//なんかこの辺でheadとneckのスケルトンデータ取得してるな
