...
void SingleFace::FTHelperCallingBack(PVOID pVoid)
{
    ...
    if (pApp)
    {
        ...
        if (pResult && SUCCEEDED(pResult->GetStatus()))
        {
            ...
            pResult->Get3DPose(&scale, rotationXYZ, translationXYZ);//ここに顔の向きが入ってるみたい
...
//int APIENTRY aa(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR lpCmdLine, int nCmdShow)
